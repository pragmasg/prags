...
function ServiceRow({ number, name, desc, learnMore, href, isLast }: ServiceRowProps) {
  const [hovered, setHovered] = useState(false);

  return (
    <a
      href={href}
      className={`group block border-t border-[#E5E5E5] ${isLast ? 'border-b' : ''} ${
    ...
    >
      <div
        className={`border-l-2 transition-colors duration-200 ${
          hovered ? 'border-[#00D4AA]' : 'border-transparent'
        }`}
      >
  ...
  return (
    <section id="services" className="bg-white py-24 lg:py-32">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-16">
        ...
        <h2 className="text-3xl lg:text-4xl font-semibold text-[#0A0A0A] leading-tight max-w-2xl">
          {t('title')}
        </h2>
