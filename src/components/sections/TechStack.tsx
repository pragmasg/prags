  ...
  return (
    <section className="bg-[#111111] py-16 lg:py-20 border-t border-white/[0.08]">
      <div className="max-w-7xl mx-auto px-6">
        <p className="text-xs uppercase tracking-widest text-[#BBBBBB] mb-10 text-center">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-0 border border-white/[0.08]">
          {stack.map((item, i) => (
            ...
            >
              ...
              <div className="h-0.5 bg-[#F97316] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left mb-4" />

              <p className="text-[10px] uppercase tracking-widest text-[#F97316] mb-2">
                {item.category}
              </p>
              <p className="text-sm font-bold text-[#F5F5F5]">{item.name}</p>
              <p className="text-xs text-[#BBBBBB] mt-1">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
